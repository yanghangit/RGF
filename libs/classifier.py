# Modified based on https://huggingface.co/ctheodoris/Geneformer (Apache License 2.0)
...
class Classifier:
    ...
    def __init__(
        ...
        **Parameters:**
        ...
        if self.classifier == "gene":
            all_gene_class_values = set(
                pu.flatten_list(self.gene_class_dict.values()))
            missing_genes = [
                gene
    ...
    def validate_options(self):
        ...
        if sum(self.split_sizes.values()) != 1:
            logger.error(
                "Train, validation, and test proportions should sum to 1.")
            raise
            ...
            if 1 > test_size > 0:
                if attr_to_split is None:
                    ...
                    train_data_output_path = (
                        ...
                        f"{output_prefix}_labeled_train"
                    ).with_suffix(".dataset")
                    test_data_output_path = (
                        Path(output_directory) /
                        f"{output_prefix}_labeled_test"
                    ).with_suffix(".dataset")
                    data_dict["train"].save_to_disk(
                        str(train_data_output_path))
                    data_dict["test"].save_to_disk(str(test_data_output_path))
                else:
    ...
    def train_all_data(
        ...
        **Output**
        ...
        # reshuffle in case users provide unshuffled data
    ...
    def validate(
        ...
        **Parameters**
        ...
        if self.classifier == "gene":
            insuff_classes = [
                k for k, v in self.gene_class_dict.items() if len(v) < 5]
        ...
        if self.classifier == "cell":
            for i in trange(self.num_crossval_splits):
                ...
                ksplit_output_dir = os.path.join(
                    output_dir, f"ksplit{iteration_num}")
                ...
                if n_hyperopt_trials == 0:
                    ...
                    best_run, trainer = self.hyperopt_classifier(
                        model_directory,
                        num_classes,
                        train_data,
                        eval_data,
            ...
            skf = cu.StratifiedKFold3(
                n_splits=n_splits, random_state=0, shuffle=True)
            # (Cross-)validate
            test_ratio = self.oos_test_size / \
                (self.eval_size + self.oos_test_size)
        ...
        if num_classes == 2:
            ...
            all_tpr = [result["roc_metrics"]["interp_tpr"]
                       for result in results]
            all_roc_auc = [result["roc_metrics"]["auc"] for result in results]
            all_tpr_wt = [result["roc_metrics"]["tpr_wt"]
                          for result in results]
    ...
    def hyperopt_classifier(
        ...
        **Parameters**
        ...
        ray.init(runtime_env={"py_modules": [lib_dir_path]})
        ...
        if os.path.isfile(saved_model_test) is True:
            logger.error(
                "Model already saved to this designated output directory.")
            raise
        # make output directory
        subprocess.call(f"mkdir {output_directory}", shell=True)

        ##### Load training args #####
        def_training_args = cu.get_default_train_args(self.classifier, train_data)

        if self.training_args is not None:
            def_training_args.update(self.training_args)
        logging_steps = round(
            len(train_data) /
            def_training_args["per_device_train_batch_size"] / 10
        )
    ...
    def evaluate_saved_model(
        ...
        **Parameters**
        ...
        model = pu.load_model(self.model_type, num_classes,
        ...
        if num_classes == 2:
            ...
            sensitivity = result["roc_metrics"]["sensitivity"]
            specificity = result["roc_metrics"]["specificity"]
            all_roc_metrics = {
                "mean_tpr": mean_tpr,
                "mean_fpr": mean_fpr,
                "all_roc_auc": all_roc_auc,
    ...
    def plot_predictions(
        ...
        **Parameters**
        ...
        if custom_class_order is not None:
            custom_dict = dict(
                zip(custom_class_order, [
                    i for i in range(len(custom_class_order))])
            )
