...
def get_default_train_args(classifier, data):
    batch_size = 12
    if classifier == "cell":
        epochs = 10
    ...
    for subdir, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith("result.json"):
                ...
                file_dict[f"{subdir}"] = result_json["eval_accuracy"]
    file_df = pd.DataFrame(
        {"dir": file_dict.keys(), "eval_accuracy": file_dict.values()}
    )
    model_superdir = (
        "run-"
        + file_df.iloc[file_df["eval_accuracy"].idxmax()]["dir"]
        .split("_objective_")[2]
        .split("_")[0]
    )
